        ...
        if (listaPedido.size() > 0) {
            for (DetalleFacturaDAO item : listaPedido) {
                ...
                if (item.getProducto() != null) {
                    valorTotal = valorTotal.add(item.getSubTotalDescuento().multiply(item.getCantidad()));
...
//                BigDecimal valorIva = subTotalCotizacion.multiply(parametrizar.getParIva());
                ...
                valorTotalCotizacion = valorTotalConIva;
    ...
    private void guardarNotaCredito() {
        ...
        try {
            ...
            if (listaPedido.size() > 0) {
                ...
                creditoDebito.setFacSubtotal(subTotalCotizacion);
                creditoDebito.setFacTipo("NCRE");
                creditoDebito.setFacTotal(valorTotalCotizacion);
                creditoDebito.setFacTotalBaseCero(BigDecimal.ZERO);
                creditoDebito.setFacTotalBaseGravaba(factura.getFacSubtotal());
